...
export type Bin = (opts: BinOptions) => Promise<number>
export interface BinOptions {
  args: string[]
  terminal: History
  system: System
}
export const bin: {[title: string]: Bin} = {}
...
export class CommandTerminal extends PolymerElement {
  ...
  connectedCallback() {
    ...
    const commandLine = this.$.cli as HTMLFormElement
  ...
  activeCallback() {
    const input = this.$.entry as HTMLElement
    input.focus()
  }

  handleCmd() {
    const input = this.$.entry as HTMLInputElement
