...
test('Editing a 4KB file', t => {
  ...
  t.deepEqual(perfHdd, {
    readTime: 5000010, // 0.005s
    writeTime: 5000010,
    energy: 32768000000163.84, // 32J
    power: 0.000009102222222267733 // kWh
  })
  const perfSsd = resourceMonitor.calcPerf(Arch.SYS_SSD)
  t.deepEqual(perfSsd, {
    readTime: 100010, // 0.0001s,
    writeTime: 100010,
    energy: 32768163.84, // 0.000032J
...
test('Editing video clips', t => {
  ...
  const perfHdd = resourceMonitor.calcPerf(Arch.SYS_HDD)
  t.deepEqual(perfHdd, {
    readTime: 15000030, // 0.005s
    writeTime: 5000010,
    energy: 125829120000629150, // 32J
    power: 0.0349525333335081 // kWh
  })
  const perfSsd = resourceMonitor.calcPerf(Arch.SYS_SSD)
  t.deepEqual(perfSsd, {
    readTime: 300030, // 0.0001s,
    writeTime: 100010,
    energy: 125829749145.6, // 0.000032J
...
test('Training data for ML', t => {
  ...
  t.deepEqual(perfHdd, {
    readTime: 320000640, // 0.32s
    writeTime: 5000010,
    energy: 419430400002097150, // 419430J
    power: 0.11650844444502699 // kWh
  })
  const perfSsd = resourceMonitorStd.calcPerf(Arch.SYS_SSD)
  t.deepEqual(perfSsd, {
    readTime: 6400640, // 0.006s,
    writeTime: 100010,
    energy: 419432497152, // 0.419J
